 ...
 *
 * Description: Operator Placement Problem (OPP): standard PL{01} formulation 
 *				in Mathematical Programming, as stated in "Posizionamento degli
 * 				Operatori" by stud. Giacomo Marciani.
 *********************************************************************************/
...
tuple NodeDSP {
	...
	int dimIn;			// [bits/tuple] tuples in dimension
	int dimOut;			// [bits/tuple] tuples out dimension
};

tuple EdgeDSP {
	key NodeDSP src; 	// [identifier] source node
	key NodeDSP dst; 	// [identifier] destination node	
	float flow; 		// [tuples] tuples flowing through
	int dim;			// [bits/tuples] tuples through dimension
};
...
sum (nodeDSP in nodesDSP: nodeDSP.id in pathDSP.nodes)
	sum (nodeRES in nodesRES: eligiblePins[nodeDSP][nodeRES] == 1)
		placeNode[nodeDSP][nodeRES] * (nodeDSP.flowIn / (nodeRES.speedup * nodeDSP.execution));
...
sum (edgeDSP in edgesDSP: edgeDSP.src.id in pathDSP.nodes diff {last(pathDSP.nodes)} && edgeDSP.dst.id in pathDSP.nodes diff {first(pathDSP.nodes)})
	sum (edgeRES in edgesRES: eligiblePins[edgeDSP.src][edgeRES.src] == 1 && eligiblePins[edgeDSP.dst][edgeRES.dst] == 1)
		placeEdge[edgeDSP][edgeRES] * ((edgeDSP.flow * edgeDSP.dim / edgeRES.bandwidth) + edgeRES.delay);
...
wR * ((maxR - R)/(maxR - minR)) + wA * ((A - log(minA))/(log(maxA) - log(minA)));
...
subject to {
	...
	* ********************************************
 	...
 	*********************************************/
	forall (edgeDSP in edgesDSP, edgeRES in edgesRES: 
	eligiblePins[edgeDSP.src][edgeRES.src] == 1 && eligiblePins[edgeDSP.dst][edgeRES.dst] == 1)
		connectivity_one:
		placeNode[edgeDSP.src][edgeRES.src] + 
		placeNode[edgeDSP.dst][edgeRES.dst] - 1 <= 
	...
	eligiblePins[edgeDSP.src][edgeRES.src] == 1 && eligiblePins[edgeDSP.dst][edgeRES.dst] == 1)
		connectivity_two:
		placeNode[edgeDSP.src][edgeRES.src] + 
		placeNode[edgeDSP.dst][edgeRES.dst] >= 
		2 * placeEdge[edgeDSP][edgeRES];	
}
